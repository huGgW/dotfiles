...
Local snapshot of the official Notion enhanced markdown reference.

- Source: https://developers.notion.com/guides/data-apis/enhanced-markdown.md
- Fetched: 2026-05-13
- Purpose: Provide a local reference for the `notion-formatter` skill so agents do not fetch the same spec on every invocation.
...
Do not escape special characters inside code blocks. Set the language if known. Use `mermaid` as the fenced code language for Mermaid diagrams.
